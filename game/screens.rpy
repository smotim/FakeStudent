...
screen main_menu():
    ...
    imagemap:
            ...
            hotspot (1152, 418, 163, 48) action Start()
            hotspot (1344, 483, 217, 45) action ShowMenu("load")
            hotspot (1180, 552, 252, 49) action ShowMenu("preferences")
            hotspot (1341, 634, 201, 48) action Help()
            hotspot (1186, 678, 133, 48) action Quit(confirm=True)
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    textbutton "Arabic" text_font "DejaVuSans.ttf" action Language("Arabic")
...
screen map():
    ...
    imagemap:
        ground "map"
        hotspot ((288, 485, 168, 140)) hovered tt.Action("Гук") action Jump("guk")#на карте номер 1
        hotspot ((477, 523, 110, 100)) hovered tt.Action("ИНЭУ") action Jump("ineu")#на карте номер 2
        hotspot ((152, 870, 105, 95)) hovered tt.Action("АСУ") action Jump("stroika")#на карте номер 4 
        hotspot ((480, 311, 116, 92)) hovered tt.Action("Теплофак") action Jump("teplofuck")#на карте номер 5
        hotspot ((815, 615, 116, 99)) hovered tt.Action("ФТИ") action Jump("fizteh")#на карте номер 6
        hotspot ((1433, 758, 99, 89)) hovered tt.Action("РТФ") action Jump("chempion")#на карте номер 7 
        hotspot ((1440, 350, 129, 130)) hovered tt.Action("Стадион") action Jump("label_8")#на карте номер 8
        hotspot ((1134, 91, 139, 114)) hovered tt.Action("Общежитие") action Jump("dormitory")#на карте номер 9
        hotspot ((1240, 826, 121, 131)) hovered tt.Action("ИНМиТ") action Jump("inmt")#пока без номера
    text tt.value
screen dictionaryButton:
    vbox xpos 1750 ypos 10:
        style_prefix "blue"
        textbutton "Словарь" action Jump("dict")
#этот алгоритм собираемся сделать по-нормальному
screen dictionary(): 
    frame:  
        vbox:
            style_prefix "green"
            if studik:
                text "Студик - сокращение от \"Студенческий билет\", документ который смотрит охранник, когда ты заходишь в институт"
            else:
                text "Студик - ????"
            if alaska:
                text "\n1.Аляска - это места в аудитории, которые находятся далеко от преподавателя."
            else:
                text "\n1.Аляска - ????"
            if zabil:
                text "\n2.Забить - значит перестать уделять внимание чему-то. Студенты часто забивают на какой-нибудь предмет или задание"  
                #"
            else:
                text "\n2.Забить - ????"
            if bomb:
                text "\n3.Бомбами студенты часто называют шпоргалки. Если услышите, что студент готовит бомбу, скорее всего речь о записке подсказками"
                 #"
            else:
                text "\n3.Бомбы - ????"
            if skovorodka:
                text "\n4.Сковородка - это место рядом с общежитиями УрФУ. Это слово используют только стдуденты нашего университета. Сковородка будет добавлена в игру в следующих обновлениях ;)"
            else:
                text "\n4.Сковородка - ????"
...
screen browser():
        imagemap:
            ...
            hotspot (502, 188, 928, 302) action Jump("googling")
...
screen vk_mc_name():
    vbox:
        ...
        text _("{color=#000}[name]{/color}"):
            size 24
screen laptop:
    vbox xpos 1750 ypos 100:
        style_prefix "blue"
        textbutton "Ноутбук" action Jump("lapt")
screen dormitory_label:
    vbox:
        xalign 0.5 yalign 0.5
        text  _("{color=#000}Общежитие{/color}"):
            size 120
